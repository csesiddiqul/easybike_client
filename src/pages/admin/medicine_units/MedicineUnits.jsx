...
import {
  useDeleteMedicineUnitsMutation,
  useGetMedicineUnitsQuery,
} from "../../redux/api/medicineUnitsApiSlice";
...
const MedicineUnits = () => {
  ...
  // handle fetching
  const { data, isLoading, refetch } = useGetMedicineUnitsQuery({
    page: currentPage,
  ...
  if (
    somePermission([
      Utils.permissions.edit_medicine_unit,
      Utils.permissions.delete_medicine_unit,
    ])
  ) {
    columns.push({
      label: "Action",
      ...
      render: (value, tableMeta) => (
        <div className="flex items-center gap-2">
          {hasPermission([Utils.permissions.edit_medicine_unit]) && (
            <Button onClick={() => handleEdit(tableMeta?.rowData)}>
              <FiEdit />
            </Button>
          )}
          {hasPermission([Utils.permissions.delete_medicine_unit]) && (
            <Popconfirm
  ...
  return (
    <div className="card-layout">
      <div className="flex justify-between headerbg items-center py-2 mb-4">
        ...
        {hasPermission([Utils.permissions.create_medicine_unit]) && (
