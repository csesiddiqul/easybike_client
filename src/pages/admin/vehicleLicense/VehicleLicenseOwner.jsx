...
import { Card, Spin, Button, Checkbox, Tag, message } from "antd";
import {
...
const VehicleLicense = () => {
  ...
  const ownerId = userInfo?.owner?.id;
  ...
  // ---------------- API ----------------
  const { data, isLoading, refetch } = useGetMakeVehicleLicensePaymentQuery(
    { owner_id: ownerId, phone: "", status },
    { skip: !ownerId } // fetch only if owner exists
  );
  ...
  const items = useMemo(
    () =>
      Object.values(selectedLicences)
        .flat()
        .map((l) => ({ licence_id: l.licence_id })),
    [selectedLicences]
  );
  ...
  useEffect(() => {
    if (ownerId) {
      refetch(); // page load e automatically hit
    }
  }, [ownerId, refetch]);
